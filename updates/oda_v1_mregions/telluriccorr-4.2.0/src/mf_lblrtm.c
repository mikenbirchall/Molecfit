...
delta=delta/24.0;
                    ...
                    wn_jmax = j;
                    cpl_msg_info(cpl_func,"MNBX wn_jmax=%lld",wn_jmax);
                }
                ...
                cpl_msg_info(cpl_func,"MNBX config_lblrtm->v[0]=%f config_lblrtm->v[1]=%f",config_lblrtm->v[0],config_lblrtm->v[1]);
                cpl_msg_info(cpl_func,"MNBX minc_config=%f maxc_config=%f",minc_config, maxc_config);
                cpl_msg_info(cpl_func,"MNBX fabs=%f,MF_LBLRTM_DELTA_ABS=%f",fabs(maxc_config-minc_config), MF_LBLRTM_DELTA_ABS);
...
delta=delta/24.0;
                        ...
                        cpl_msg_info(cpl_func,"MNBX delta=%f min_wn_local=%f : %f %f",delta,min_wn_local,wn1[range],wn2[range]);
        ...
        for (cpl_size wavenumber = 0; wavenumber < size - 1 && !err; wavenumber++) {
            ...
            if (klim[1] <= klim[0]) {
                ...
                if (llim[1] < lamv[0] || llim[0] > lamv[nrow - 1]) {
                    ...
                    err = mf_io_read_lblrtm_and_update_spec(nrow, lamv, fluxv, spectrum_filename, llim, &usampl, &jmin, &jmax);
