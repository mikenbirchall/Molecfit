...
cpl_array* mf_derive_continuum_pars (
    const mf_parameters  *params,
    const cpl_array *fitpar,
    cpl_table *rangespec,
    const int range);
...
*/
            ...
            /* Define new fitpar2 with updated contiuum parameters solved for via SVD*/
            cpl_array* fitpar2 = mf_derive_continuum_pars(params,fitpar,rangespec,j+1);
...
cpl_array* mf_derive_continuum_pars (const mf_parameters  *params, const cpl_array *fitpar, cpl_table *rangespec, const int range) {
    ...
    for (cpl_size k=0;k<nlam;k++)  {
        ...
        if (debug && k<5) {
            ...
            double v4=cpl_matrix_get(A,k,3);
            double v5=cpl_matrix_get(A,k,4);
            cpl_msg_info(cpl_func,"--->MNG DEBUG: A(%lld,1:5) = %f,%f,%f,%f,%f",k,v1,v2,v3,v4,v5);
        }
    }
