    ...
    const mf_io_lnfl_config  *config)
{
  ...
  cpl_boolean TAPE5_FLAG=mf_QWKAParseEnvVar(TAPE5);
  if (TAPE5_FLAG) cpl_msg_info(cpl_func,">>>>>>>>>>>>>>>>>>>>>>TAPE 5 FLAG CALLED>>>>>>>>>>>>>>>>>>>>>");
    ...
    const cpl_table          *prof)
{
    ...
    if (!stream) {
        ...
        if (fprintf(stream, "    %1i    %1i    %1i    %1i    %1i    %1i    %1i"
                    ...
                    "\n", 1, 1, config->icntnm, config->iaersl, 0, 0, 0, 0, 0, 1, " 0", 0, 0,
