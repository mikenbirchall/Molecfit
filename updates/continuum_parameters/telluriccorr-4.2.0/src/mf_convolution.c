...
static cpl_array* mf_derive_continuum_pars (
    cpl_table *rangespec,
    const mf_parameters  *params,
    const cpl_array *fitpar,
    const int range);
    ...
    for (cpl_size j = 0; j < nrange; j++) {
	    ...
	    if (!modspec) cpl_msg_info(cpl_func,"!not modspec so returning err");
            ...
            */
            ...
            cpl_array *fitpar_null=mf_derive_continuum_pars (rangespec,params, fitpar, j+1);
            if (fitpar_null!=NULL) cpl_msg_info(cpl_func,"NOT NULL OK");
...
static cpl_array* mf_derive_continuum_pars (cpl_table *rangespec, const mf_parameters  *params, const cpl_array *fitpar, const int range) {
    ...
    /* Make fitpar2 initially a copy of fitpar*/
    cpl_array *fitpar2 = cpl_array_duplicate(fitpar);

    if (debug) cpl_msg_info(cpl_func,"---> MNB-DEBUG: Inside Auto Derive Continuum Pars");
    /* Sanity Check */
    if (ncont - 1 < 0) {
        cpl_msg_info(cpl_func,"---> MNB-DEBUG: Inside Auto Derive Continuum Pars AND THERES A APROBLEM");;
    }
    ...
    double limlam[2] = { cpl_table_get( rangespec, MF_COL_IN_LAMBDA,  0,        NULL),
                         cpl_table_get( rangespec, MF_COL_IN_LAMBDA,  nlam - 1, NULL) };
    double wmean = (limlam[0] + limlam[1]) / 2;

    if(debug) cpl_msg_info(cpl_func,"MNB lam0=%f lam1=%f wmean=%f %lld,%d",limlam[0],limlam[1],wmean,nlam,ncont);
    ...
    for (cpl_size k=0;k<nlam;k++)  {
        ...
        if (val>=0.0 && wt1>0.0 && wt2>=0.0) {
            cpl_msg_info(cpl_func,"NONE zeros at k=%lld %f %f %f",k,val,wt1,wt2);
            break;
        }
    }
    ...
    for (cpl_size k=0;k<nlam;k++)  {
        ...
        double wt1 = cpl_table_get(rangespec, MF_COL_WEIGHT,   k, NULL);
      /*  double wt2 = cpl_table_get(rangespec, MF_COL_MOD_WEIGHT,k, NULL);*/
        double wt  = wt1  ;/*wt1*wt2;*/
        cpl_matrix_set(RHS,k,0,val);
        cpl_matrix_set(W  ,k,k,wt);
        if (debug && k<5) cpl_msg_info(cpl_func,"--->MNG DEBUG: RHS and W of %lld = %f %f",k,val,wt);
    ...
    /* Define the A matrix */
    for (cpl_size k=0;k<nlam;k++)  {
        double flx=cpl_table_get(rangespec, MF_COL_IN_FLUX,   k, NULL);
        double lam=cpl_table_get(rangespec, MF_COL_IN_LAMBDA, k, NULL);
        double slam=lam-wmean;
        double val=flx;
        ...
        if (debug && k<5) {
            double v1=cpl_matrix_get(A,k,0);
            double v2=cpl_matrix_get(A,k,1);
            double v3=cpl_matrix_get(A,k,2);
            cpl_msg_info(cpl_func,"--->MNG DEBUG: A(%lld,1:5) = %f,%f,%f",k,v1,v2,v3);
        }
    }
...
/*    cpl_matrix* WA   =cpl_matrix_product_create(W,A);
    ...
    for (int i=0; i<nlam; i++) {
        ...
        for (int j=0; j<ncont; j++) {
            double Aij =cpl_matrix_get(A,i,j);
            double SOLj=cpl_matrix_get(SOL,j,0);

            sum=sum+Aij*SOLj;
        }
      /* double RHSi=cpl_matrix_get(RHS,i,0);*/
       double tflx=cpl_table_get(rangespec, MF_COL_IN_FLUX,i, NULL);
       double tmp=1.0*tflx;
       dev=dev+pow(tmp,2);
    }
    cpl_msg_info(cpl_func,"dev=%f", dev);
    SOL=cpl_matrix_solve_normal(A,RHS);
    if(debug) cpl_matrix_dump(SOL,NULL);

    cpl_array_delete(fitpar2);
    cpl_matrix_delete(A);
    cpl_matrix_delete(W);
    ...
    /* Update the contiuum parametersof fitpar2 */
    for  (int l1=0;l1<ncont;l1++) {
        double sval=cpl_matrix_get(SOL,l1,0);
        double fval=par[n0 + l1];
        if (debug) cpl_msg_info(cpl_func,"RANGE %d i=%d sol=%f, cof=%f cp %f",range,l1,sval,fval,cpl_array_get_double(fitpar2,n0+l1,NULL));
 ...
 /*   cpl_matrix_delete(WA);
    ...
    if (debug) cpl_msg_info(cpl_func,"---> MNB-DEBUG: Finished Auto Derive Continuum Pars");
