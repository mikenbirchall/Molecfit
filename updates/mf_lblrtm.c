                ...
                for (cpl_size range = 0; range < nrange && !err; range++) {
                      ...
                      if (execute_range[range]) {
                          ...
                          for (cpl_size wavenumber = 0; wavenumber < num_wavenumber[range]; wavenumber++) {
                              ...
                              /*char *cmd_str = cpl_sprintf("cp %s %s",old_output_name,new_output_name);
                              cpl_msg_info(cpl_func,">>>>>>MNB Attempt to system call %s",cmd_str);
                              err = mf_io_system(cmd_str,NULL,NULL);
                              cpl_free(cmd_str);*/
