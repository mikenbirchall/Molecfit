...
{
/* MNB QWK MF INSERT FROM HERE */
    cpl_msg_info(cpl_func,"             ==============");
    cpl_msg_info(cpl_func,"             QWK MF VARIANT");
    cpl_msg_info(cpl_func,"             ==============");
    cpl_boolean VERBOSE_FLAG  =mf_QWKAParseEnvVar(VERBOSE);
    cpl_boolean TERMINATE_FLAG=mf_QWKAParseEnvVar(TERMINATE);
    if ( VERBOSE_FLAG)  cpl_msg_info(cpl_func,"---> VERBOSE");
    if (!VERBOSE_FLAG)  cpl_msg_info(cpl_func,"---> NOT VERBOSE");
    if (TERMINATE_FLAG) return CPL_ERROR_NONE;
/* MNB QWK MF INSERT TO HERE */
  ...
  vars.lblrtm_config = config_lblrtm;
  ...
  if (status < 0) {
      ...
      if (!err) {
          err = mf_lblrtm(     run_execution,
                               ...
                               vars.reffitpar, results->mpfit_calls, vars.lblrtm_calls, config_lblrtm,
    ...
    void                     *generic_vars)
{
  ...
  mf_io_lblrtm_config *config_lblrtm    = vars->lblrtm_config;
  ...
  cpl_error_code err = mf_lblrtm(run_execution,
                                 params, last_call, spec_out, range_status,
                                 reffitpar, *mpfit_calls, lblrtm_calls, config_lblrtm,
                                 atm_profile, atm_profile_out,
                                 fitpar);
